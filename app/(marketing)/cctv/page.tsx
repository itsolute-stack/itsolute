...
import { cctvCopy, cctvFAQs } from '@/lib/content/copy/cctv'
...
export default function CctvPage() {
  return (
    <>
      <PageHero
        eyebrow={cctvCopy.hero.eyebrow}
        headline={cctvCopy.hero.headline}
        sub={cctvCopy.hero.sub}
        primaryCta={{
          label: cctvCopy.hero.primaryCta,
          href: '/contact?service=cctv',
        }}
        secondaryCta={{
          label: cctvCopy.hero.secondaryCta,
          message: cctvCopy.hero.whatsappMessage,
        }}
      />
      ...
      <Section theme="light">
        <Container>
          <SectionHeader
            eyebrow={cctvCopy.install.eyebrow}
            headline={cctvCopy.install.headline}
          />
          <div className="mt-16 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 lg:gap-5">
            {cctvCopy.install.items.map((item) => {
        ...
        <Container>
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-16">
            <div className="lg:col-span-5">
              <SectionHeader
                eyebrow={cctvCopy.who.eyebrow}
                headline={cctvCopy.who.headline}
                theme="dark"
              />
            </div>
            <ul className="lg:col-span-7 flex flex-col gap-3">
              {cctvCopy.who.items.map((item) => (
                <li
                  key={item}
      ...
      <Section theme="cream">
        <Container>
          <SectionHeader
            eyebrow={cctvCopy.why.eyebrow}
            headline={cctvCopy.why.headline}
          />
          <div className="mt-16 grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
            {cctvCopy.why.points.map((p) => (
      ...
      <Section theme="dark">
        <Container>
          <SectionHeader
            eyebrow={cctvCopy.process.eyebrow}
            headline={cctvCopy.process.headline}
            theme="dark"
          />
          <ol className="mt-16 grid grid-cols-1 gap-10 md:grid-cols-5 md:gap-6">
            {cctvCopy.process.steps.map((step, i) => (
              <li key={step.index} className="relative flex flex-col gap-3">
                <div className="flex items-center gap-3">
                  ...
                  {i < cctvCopy.process.steps.length - 1 ? (
                    <span
                      aria-hidden
      ...
      <Section theme="light">
        <Container>
          <SectionHeader
            eyebrow={cctvCopy.tiers.eyebrow}
            headline={cctvCopy.tiers.headline}
          />
          <div className="mt-16 grid grid-cols-1 gap-4 md:grid-cols-3 lg:gap-5">
            {cctvCopy.tiers.items.map((tier) => (
              <div
                key={tier.title}
                className={cn(
              ...
              >
                ...
                <Button asChild className="mt-auto" variant={tier.highlight ? 'primary' : 'secondary'}>
                  <Link href="/contact?service=cctv">Request a site survey</Link>
                </Button>
              </div>
            ))}
          </div>
          <p className="mt-8 max-w-2xl text-sm text-slate-500 leading-relaxed">
            {cctvCopy.tiers.disclaimer}
          </p>
        </Container>
      </Section>
      ...
      {/* Final CTA */}
      <FinalCTA
        headline={cctvCopy.finalCta.headline}
        sub={cctvCopy.finalCta.sub}
        primary={cctvCopy.finalCta.primary}
        secondary={cctvCopy.finalCta.secondary}
        whatsappMessage={cctvCopy.hero.whatsappMessage}
      />
      ...
      <Script
        ...
        type="application/ld+json"
        strategy="afterInteractive"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(faqSchema(cctvFAQs)) }}
      />
    </>
  )
}
